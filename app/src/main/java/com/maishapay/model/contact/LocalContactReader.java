...
public class LocalContactReader {
    ...
    public Map<String, MaishapayContact> getContacts(Account account) {
        ...
        try {
            ...
            Map<String, MaishapayContact> contacts = new HashMap<>();

            while (rawContactsCursor.moveToNext()) {
                long rawContactId = rawContactsCursor.getLong(0);
                MaishapayContact contact = getContact(rawContactId);
    ...
    private MaishapayContact getContact(long rawContactId) {
        ...
        try {
            ...
            MaishapayContact contact = new MaishapayContact();
            ...
            while (cursor.moveToNext()) {
                ...
                if (!cursor.isNull(1)) {
                    ...
                    if (StructuredName.CONTENT_ITEM_TYPE.equals(mimeType))
                        contact.displayName = cursor.getString(3);
                    if (CommonDataKinds.Email.CONTENT_ITEM_TYPE.equals(mimeType))
                        contact.email = cursor.getString(3);
                    if (CommonDataKinds.Phone.CONTENT_ITEM_TYPE.equals(mimeType) && CommonDataKinds.Phone.TYPE_WORK_MOBILE == subType)
                        contact.phoneNumber = cursor.getString(3);
                    if (CommonDataKinds.Phone.CONTENT_ITEM_TYPE.equals(mimeType) && CommonDataKinds.Phone.TYPE_WORK == subType)
                        contact.fixedPhoneNumber = cursor.getString(3);
                    if (CommonDataKinds.Phone.CONTENT_ITEM_TYPE.equals(mimeType) && CommonDataKinds.Phone.TYPE_OTHER == subType)
                        contact.shortPhoneNumber = cursor.getString(3);
                }
            }
